...
created: 2026-05-12
description: "Akshay @akshay_pachaar·Apr 6 ArticleThe Anatomy of an Agent Harness A deep dive into what Anthropic, OpenAI, Perplexity and LangChain are ac..."
tags:
  - "clippings"
ingested-at: 2026-05-16
---
![Image](https://pbs.twimg.com/media/HH_SeVYXMAA0-k4?format=jpg&name=large)

> Apr 6

作者：[@akshay\_pachaar](https://x.com/@akshay_pachaar)
...
![Image](https://pbs.twimg.com/media/HH_X7DiWQAASlWi?format=jpg&name=large)

Beren Millidge 在其 2023 年的博文中做了一个精准的类比：原生大语言模型就像一个没有内存、没有硬盘、也没有输入输出设备的 CPU。此时，**上下文窗口**充当了内存（快但容量有限），**外部数据库**扮演了硬盘（大但速度慢），**工具集成**则是设备驱动程序。而**Harness**，就是那个操作系统。正如 Millidge 所写：“我们重新发明了冯·诺依曼架构（Von Neumann architecture）”，因为这是任何计算系统最自然的抽象方式。
...
![Image](https://pbs.twimg.com/media/HH_YZYdWkAA-BtD?format=jpg&name=large)
...
Claude Code 提供了六大类工具：文件操作、搜索、执行、网页访问、代码分析和子智能体创建。OpenAI 的 Agents SDK 则支持函数工具（通过 [@function\_tool](https://x.com/@function_tool) 定义）、托管工具（如网页搜索、代码解释器、文件搜索）以及 MCP **（Model Context Protocol，一种开放的工具接入标准）** 服务器工具。
...
现代 Harness 依赖于**原生工具调用**，即模型返回结构化的 tool\_calls 对象，而不是需要费力解析的自由文本。Harness 会检查：是否有工具调用？如果有，执行并继续循环；如果没有，那当前的输出就是最终答案。
...
![Image](https://pbs.twimg.com/media/HH_YfilX0AAFR4G?format=jpg&name=large)
...
![Image](https://pbs.twimg.com/media/HH_YlPbWgAAyPU9?format=jpg&name=large)
...
![Image](https://pbs.twimg.com/media/HH_YrOyXUAAOFUR?format=jpg&name=large)
...
![Image](https://pbs.twimg.com/media/HH_Yw1OWAAAphY9?format=jpg&name=large)

每个 Harness 的架构师都面临这七个选择：

![Image](https://pbs.twimg.com/media/HH_Y2gpWcAAl2uX?format=jpg&name=large)
