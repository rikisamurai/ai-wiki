...
这是 [[wiki/aigc/coordinator-模式|Coordinator 模式]] 能成立的前提——**经理只能有一个**这条不变量靠 prompt 强制注入到每个 worker。同时还有一个缓存优化的副产品：所有 fork worker 的工具结果统一替换成占位符 `'Fork started — processing in background'`，使 N 个并行 worker 的前缀完全一致，**只有第一个冷启动，后面 N-1 个直接命中缓存**。
...
**经理只能有一个**这个不变量是写在 prompt 而非代码里的——因为它需要被模型理解才能遵守。这与 [[wiki/aigc/coordinator-模式|Coordinator 模式]] 的"主控分发"配套使用：主 Agent 是经理，所有 sub-agent 强制成为 worker。
