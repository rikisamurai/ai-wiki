...
> [!important] 它解决了什么问题
> Claude Code 配置随时间会**累积熵**：CLAUDE.md 越写越长、skills 越装越多、hooks 配了忘了改、规则同一件事在 3 个地方都说一遍。靠人工很难发现这些问题。claude-health 把六层架构每一层的**反模式清单**做成自动检查——本质是把 [[wiki/agent-engineering/code-review/ai-写-lint|AI 写 Lint]] 的思路用在 Claude Code 配置上。
...
> claude-health 用 12.5% 作为 MCP 占 [[wiki/agent-engineering/context/context-window|context]] 开销的红线——超过就提示拆分。这个数字不是凭空来的：Anthropic 自己测过 5 个 MCP server 平均吃 25K tokens，超过窗口的 12.5% 就开始挤占代码本身的空间。**这是 [[mcp|MCP 隐性成本]]最具操作性的量化标准**。
...
**关联**：[[claude-code-六层架构|Claude Code 六层架构]]（理论基础） / [[hooks|Hooks]] / [[wiki/claude-code/auto-memory|Auto Memory]]（health 的检查项之一） / [[wiki/claude-code/mcp|MCP]]（开销估算）
