...
> [!tip] 与 [[wiki/claude-code/permission-modes|权限模式]] 配合
...
> 一个典型 MCP Server 含 20-30 个工具定义，每个 ~200 tokens。**接 5 个 server，固定开销就达 ~25,000 tokens（占 200K 上下文的 12.5%）**。所有 MCP 工具 schema 在每次请求开头被全量发送——这部分被 [[wiki/agent-engineering/context/prefix-cache|Prefix Cache]] 缓存，但**占用窗口的额度**。
>
> **优化策略**：
...
> - 会话中途**不要**临时禁用某 MCP——工具集变了 = 缓存断（参见 [[wiki/agent-engineering/context/cache-失效陷阱|cache 失效陷阱]]）
