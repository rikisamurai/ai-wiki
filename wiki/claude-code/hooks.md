...
> [!tip] Hooks 不要承担太多
> Hooks 跑在每次工具调用后，慢的 hook 会拖累整个会话节奏。把 lint 限定到改动文件而非全仓、把测试推到 commit 阶段而非 edit 阶段。重活留给 [[wiki/agent-engineering/workflow/subagent-上下文隔离|subagent]]。
...
>
> 只写 CLAUDE.md 规则，Claude 经常当没看见——因为它有"别的优先事项要赶"。Hook 把"建议性"升级为"确定性"。这是 [[wiki/agent-engineering/workflow/enforce-invariants|Enforce Invariants]] 在 Claude Code 这一层的具体落地。
