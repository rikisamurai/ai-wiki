...
- 同一 git 仓库的所有 [[wiki/skills/superpowers|worktree]] 和子目录**共享同一个** auto memory 目录
...
> Auto Memory 不是关键词搜索——Claude Code 用**另一个 Sonnet 模型**扫描所有记忆文件的标题和描述，选出**最多 5 条**最相关的全文注入当前对话。策略明确：**精确度优先于召回率**——宁可漏一个可能有用的记忆，也不塞一个不相关的污染上下文。这意味着记忆文件的 frontmatter `description` 字段质量直接决定召回——和 [[wiki/skills/skill-编写实践|Skill 描述符]] 是 if-then 条件是同一回事。
