...
> 当 Agent 在写代码时，useEffect 经常被"以防万一"地添加，恰恰是下一个竞态条件或无限循环的种子。禁用这个 Hook 迫使逻辑变得**声明式和可预测**——这是 [[wiki/ai-coding/plausible-code|似是而非代码]] 的天然防护栏。
...
| --- | --- | --- |
| 1 | [[wiki/frontend/派生状态\|派生状态]]，而非同步状态 | 直接内联计算 |
| 2 | 用数据请求库（TanStack Query 等） | 处理竞态/缓存/取消 |
| 3 | 事件处理器，而非 Effect | handler 里直接做事 |
| 4 | [[wiki/frontend/usemounteffect\|useMountEffect]] 用于一次性外部同步 | 命名清晰意图 |
| 5 | 用 [[wiki/frontend/key-重置组件\|key 重置]]，而非依赖编排 | 强制干净的重新挂载 |
