...
长列表性能优化的核心思路：**滚出屏幕的 View 不销毁，放进回收池；新数据滚入时直接复用旧 View，只更新数据**。Android RecyclerView 的设计哲学，被 [[wiki/frontend/react-native/flash-list|FlashList]] 移植到 React Native，用来取代 FlatList 的"销毁重建"模式。
...
**关联**：[[wiki/frontend/react-native/flash-list|FlashList]]（实现） / [[wiki/frontend/react-native/react-native-core-components|RN 核心组件]]（list 选型上下文） / [[wiki/frontend/react-native/pressable-vs-touchable|Pressable vs TouchableOpacity]]（item 内可点击容器）
