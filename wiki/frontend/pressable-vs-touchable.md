...
React Native 里两种主流的可点击容器选型。`TouchableOpacity` 是老 API，按下时只能改变透明度；`Pressable`（0.63+）是新一代，提供 `pressed` 状态、长按、hover 等更细的回调。新代码推荐 Pressable，但既有项目里 TouchableOpacity 仍占多数——一致性优先于"新即好"。详见 [[wiki/frontend/react-native-core-components|RN 核心组件]]。
