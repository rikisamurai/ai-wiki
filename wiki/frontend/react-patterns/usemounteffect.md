...
`useMountEffect` 是 `useEffect(callback, [])` 的命名包装，用来明确"这是一次挂载时执行的副作用"。它是 [[wiki/frontend/react-patterns/no-useeffect-rule|No useEffect 规则]] 体系下唯一允许的 Effect 用法。
...
> ```
>
> 父组件负责"什么时候挂载"，子组件假设"前置条件已满足"。这是 [[wiki/frontend/react-patterns/组件强制函数|组件强制函数]] 的典型应用。
...
- 上层规则：[[wiki/frontend/react-patterns/no-useeffect-rule|No useEffect 规则]]
- 配合模式：[[wiki/frontend/react-patterns/key-重置组件|key 重置组件]]——用 key 强制重新挂载，让 useMountEffect 重新跑
- 替代方向：[[wiki/frontend/react-patterns/派生状态|派生状态]]——能不用 Effect 就别用
