...
定制滚动条有两条互斥的实现路线：**Mock 派**接管整个滚动行为，自己用事件 + transform 重新实现一遍滚动引擎；**Overlay 派**只换皮，原生 scroll 引擎不动，浮层 thumb 被动跟随原生 scroll 事件。代表前者是 react-custom-scrollbars / react-scrollbars-custom，代表后者是 [[wiki/frontend/overlayscrollbars|OverlayScrollbars]] 和 simplebar，对应 [[wiki/frontend/overlay-scrollbar-pattern|Overlay Scrollbar 范式]]。
