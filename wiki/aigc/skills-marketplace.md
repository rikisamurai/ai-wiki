...
> 经常被低估的一点：你写的 Skill **不绑定 Claude**——同样的目录在 Cursor、Codex（通过 [[wiki/aigc/codex|`.agents/skills/`]]）、Gemini CLI、OpenCode 都能跑。这意味着：
> - **写 Skill 不是给 Claude 写**，是在给"会消费 Skills 规范的所有 agent"写
> - **跨工具切换的迁移成本极低**——只要工具支持 Skills 规范，你的资产搬过去就能用
>
> 这是 [[wiki/ai-coding/harness-engineering|Harness Engineering]]里"投资可携带的资产，不投资某家公司锁定"的具体形态。
...
>
> 通用经验：**先借鉴 prompt 写法和目录结构，再选少量真用得到的装**——全装会挤 [[wiki/ai-coding/context-window|context]]、降低 [[wiki/ai-coding/cache-命中率|cache 命中率]]。
