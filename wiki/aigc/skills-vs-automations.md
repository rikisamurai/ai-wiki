...
> Claude Code 没有正式的 "Automations" 命名，但 **[[hooks|Hooks]] 就是 automations**——`PreToolUse` / `PostToolUse` / `Stop` 等事件触发的 shell 命令。同时 [[wiki/aigc/claude-code|Claude Code]] 还能配合外部调度（cron / GitHub Actions）做时间驱动的 automation：
>
> ```bash
> # 每天早上自动跑安全扫描
...
> 4. **描述说明 Skill 做什么 + 何时使用**——这条决定 [[wiki/aigc/auto-memory|检索时能不能选中]]

**关联**：[[agent-skills|Agent Skills 规范]] / [[skill-编写实践|Skill 编写实践]] / [[hooks|Hooks]]（Claude Code 的 automation 实现） / [[wiki/aigc/codex|Codex]]
