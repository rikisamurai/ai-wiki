...
Claude HUD 是 Claude Code 的 statusline 插件，把 [[wiki/ai-coding/context-window|上下文窗口]] 健康度、speed limit 消耗、subagent 状态、tool 活动**实时显示在终端输入区下方**。它解决的核心问题：Claude Code 的"context 还剩多少、被谁占了"在原生 UI 里几乎不可见，等到 [[wiki/ai-coding/compact-vs-clear|自动 compact]] 触发时已经晚了。
...
> `/cost` 是回顾性的——它告诉你已经花了多少。HUD 是预警性的——它让你**在窗口被填满之前**做决策。这与 [[wiki/ai-coding/context-rot|Context Rot]] 的核心命题一致：模型在窗口接近上限时智商最不在线，主动管理必须**提前**。
...
> [!tip] 与 [[wiki/aigc/coordinator-模式|Coordinator 模式]]搭配
> 当主线程派出多个 subagent 并行时，HUD 的"Agent 追踪"区会显示每个 worker 的运行状态和耗时——这是少数能看到 [[wiki/ai-coding/subagent-上下文隔离|subagent]] 内部进度的方式。否则你只能等它返回最终报告才知道发生了什么。
