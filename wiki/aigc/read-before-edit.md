...
理论上模型可以"被训练成习惯先 Read 再 Edit"。但 [[wiki/ai-coding/agentic-coding|Agentic Coding]] 场景下：

- 长会话中模型会**忘记**文件被外部进程改过
- 模型可能**幻觉**文件内容，写出基于错误前提的 diff
- 训练时的"建议性"规则在压力下会被模型自己合理化绕过

把规则写进**工具实现**，模型连绕过的可能都没有——这是 [[wiki/ai-coding/enforce-invariants|Enforce Invariants]] 在 Agent 工具层的应用。
...
> - 文件删除工具要求先列出将要删除的文件
>
> 不是为了卡住 Agent，而是为了让"修改"必然基于"观察"——这与 [[wiki/ai-coding/验证驱动|验证驱动]] 的"提供反馈源"是同一逻辑的两端。
