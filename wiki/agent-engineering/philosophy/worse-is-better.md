...
> 自动补齐下一个 token 是 LLM 的原生能力——只要不限制，AI 能无限把代码编下去，产出的就是典型的 [[wiki/agent-engineering/philosophy/plausible-code|Plausible Code]]：能跑、看起来很对、bug 在路上。
...
详见 [[wiki/agent-engineering/philosophy/agentic-coding|Agentic Coding]] 与 [[wiki/agent-engineering/context/隐性知识与上下文|隐性知识与上下文]]。

## 与测试的关系

把测试本身当成黄金指标，是 New Jersey 路线的极端版本——"被一大堆测试包围"≠ 高质量代码。测试只能证明功能存在，不能证明 bug 不存在。详见 [[wiki/agent-engineering/philosophy/yagni-与-dry-反论|YAGNI 与 DRY 反论]]中关于 TDD 的反思。
