...
[[wiki/agent-engineering/philosophy/vibe-coding|Vibe Coding]] 让一个开发者干原来三个人的活，对按"开发者人数"计费的 [[wiki/business/per-seat-licensing|per-seat SaaS]] 来说，面临的不是增长放缓，而是**结构性的收入萎缩**——同样的产出需要更少的席位。这是 [[wiki/agent-engineering/philosophy/vibe-coding-的代价|Vibe Coding 的代价]] 系列里"对外部生态的代价"的一面。

> [!note] 三个传导路径
> 1. **AI 默认开源偏好**：ChatGPT/Claude 生成 UI 代码时默认推 MIT 协议的 [[wiki/frontend/ui-libraries/shadcn-ui|shadcn/ui]]、Radix UI、TanStack Table，商业组件库的"默认被选中率"下降
> 2. **[[wiki/frontend/ui-libraries/headless-ui|Headless 趋势]]加速**：纯逻辑库 + AI 自动生成样式层，配置成本翻转，商业完整 styled 组件失去优势
...
> [!compare] 对比一：[[wiki/frontend/web-platform/vercel|Vercel]] vs Netlify（有 AI 叙事 vs 没有）
...
[[wiki/business/per-seat-licensing|按席位授权]] 在前端组件库（AG Grid、MUI）受冲击最快，但任何按"开发者数 × 价格"计费的 SaaS（CI、APM、低代码、IDE 插件）都在同一条曲线上，只是时间问题。
