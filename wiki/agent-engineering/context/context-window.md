...
**与上下文管理的关系**：Context Window 是物理容器，[[会话管理动作|/continue、/rewind、/clear、/compact、subagent]] 是操作工具。物理上限给定后，差距就出在操作姿势上——不会用工具 → 撞窗口上限 → 自动 compact → 翻车；用得熟 → 主动管理窗口 → 长会话也能保持质量。

**与 Prefix Cache 的关系**：每次请求发出去的"窗口里的全部内容"决定了 [[prefix-cache|Prefix Cache]] 命中情况。窗口前段稳定 → 缓存命中 → 便宜+快；窗口前段一变 → 缓存全废 → 贵+慢。所以"管理上下文窗口"和"管理缓存"在工程上是同一件事的两面。
