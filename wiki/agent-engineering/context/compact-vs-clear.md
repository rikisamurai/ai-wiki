...
> 默认压缩算法会把早期 Tool Output 和文件内容优先删掉，连带丢失架构决策。在 [[wiki/claude-code/claude-code-memory|CLAUDE.md]] 中写明压缩优先级，强制保留关键信息：
> ```markdown
...
> ```
> 这是 compact 自动机制的"配置层"。手动机制是 [[wiki/claude-code/handoff-md|HANDOFF.md]]——任务进入新阶段时，让 Claude 把进度写进文件再 `/clear`。
