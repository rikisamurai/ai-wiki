...
**在 Claude Code 里的具体实现**：见 [[claude-code-memory|Claude Code Memory 体系]]——CLAUDE.md（=AGENTS.md）+ [[claude-rules|.claude/rules/]] 模块化拆分 + [[auto-memory|Auto Memory]] 自学。Codex 用 `docs/` 目录，Claude Code 用 `.claude/rules/`，本质都是同一种"入口 + 按需加载"结构。

> [!compare] Codex 的三层 AGENTS.md 加载顺序
> Codex 官方推荐的层级（与 [[settings-scopes|Settings Scopes]] 同构，**就近原则**）：
> | 层级 | 位置 | 用途 |
> |---|---|---|
> | **个人全局** | `~/.codex/AGENTS.md` | 跨所有项目的个人偏好 |
> | **仓库根** | `<repo>/AGENTS.md` | 团队共享的项目标准（git 跟踪） |
> | **子目录** | `<repo>/<sub>/AGENTS.md` | 局部规则，更具体的优先 |
>
> Claude Code 现在也支持层级化 `CLAUDE.md`：启动时读取 cwd 向上目录的文件，读取子目录文件时再按需加载更近的 `CLAUDE.md`。[[claude-rules|.claude/rules/]] 仍适合承载 path-scoped 规则模块，不再是子目录 `CLAUDE.md` 的替代品。

> [!tip] "犯错两次就 retrospective"是 Codex 的 [[wiki/agent-engineering/workflow/两次纠正规则|两次纠正规则]]
> Codex 官方建议：**当 Codex 在同一处犯第二次错时，让它做 Retrospective 并更新 AGENTS.md**。这与 Claude Code 的 [[wiki/agent-engineering/workflow/两次纠正规则|两次纠正规则]] 是同一思路——把"反复纠正"提升为"持久化指令"。两次是阈值；一次是巧合，三次是失职。
