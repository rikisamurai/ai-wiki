...
Writer/Reviewer 模式是**多会话并行**的一种典型用法：一个 Claude 会话负责实现，另一个负责审查，二者通过你（或 Agent Teams 自动协调）来回传递。它不只是"两个会话"，而是利用**[[wiki/agent-engineering/workflow/subagent-上下文隔离|独立上下文]]**避免"自己审自己"的盲点。
...
**与 [[wiki/agent-engineering/workflow/subagent-上下文隔离|Subagent]] 的区别**
...
- **桌面应用**：可视化管理多会话，每个会话独立 [[wiki/skills/superpowers|worktree]]
- **Web 版**：Anthropic 云基础设施的隔离 VM
- **Agent Teams**：多会话自动协调，共享任务和消息队列

> [!example] 异系模型 reviewer：[[wiki/claude-code/codex-plugin|Codex Plugin]]
