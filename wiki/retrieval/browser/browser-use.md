...
Browser-Use 是 AI Agent 操作真实浏览器的总称——既指开源库 [browser-use](https://github.com/browser-use/browser-use)（Python），也指更广义的"让 LLM 看着截图点按钮"的范式。它和 [[opencli|OpenCLI]]、[[wiki/retrieval/browser/agent-browser|agent-browser]] 同属 [[cdp|CDP]] 之上的应用层。
...
> | **专用 Agent 派** | [[wiki/retrieval/browser/agent-browser\|agent-browser]] | 浏览器层针对 agent 优化（无 UI 干扰、专用 selectors） |
>
...
>
> 缓解：[[agent-browser|agent-browser]] 的 `--content-boundaries` 用 nonce 包裹外部内容；browser-use 本身**没有内置等价机制**——这是用 browser-use 跑生产 Agent 时必须自己加一层的安全空白点。参考 [[wiki/claude-code/fail-closed-tool-defaults|Fail-Closed 工具默认]]。
