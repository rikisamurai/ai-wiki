...
> ```
>
> 是 LangGraph 的默认骨架，参考 [[wiki/agent-engineering/workflow/long-horizon-agent|Long Horizon Agent]]。

> [!example] 多智能体并行
> 多个 agent 在不同系统中并行搜索，最后由聚合器合并——本质就是 [[wiki/agent-engineering/workflow/subagent-上下文隔离|Subagent 上下文隔离]] 的应用。
>
> Deep Research 就是这种模式：把"研究 X"拆成 N 个子方向，每个 subagent 独立搜+总结，主 agent 合并成最终答案。和 [[wiki/agent-engineering/workflow/subagent-driven-development|Subagent-Driven Development]] 同源——只是任务从"写代码"换成"找信息"。
...
Agentic RAG ≈ "把检索作为唯一动作的 [[wiki/agent-engineering/workflow/long-horizon-agent|Long Horizon Agent]]"。所以 Long Horizon Agent 的所有挑战在 Agentic RAG 里同样存在：

- [[wiki/agent-engineering/context/context-window|上下文窗口]]爆炸（每轮观察都进上下文）
- [[wiki/agent-engineering/context/cache-命中率|Cache 命中率]]（多轮迭代的 prompt 前缀稳定性）
- [[wiki/agent-engineering/workflow/coordinator-模式|Coordinator 模式]]（协调多 subagent 的结果）
...
**关联**：[[rag|RAG]] / [[hybrid-retrieval|混合检索]] / [[graph-rag|GraphRAG]] / [[wiki/agent-engineering/workflow/long-horizon-agent|Long Horizon Agent]] / [[wiki/agent-engineering/workflow/subagent-上下文隔离|Subagent 上下文隔离]] / [[coordinator-模式|Coordinator 模式]]
